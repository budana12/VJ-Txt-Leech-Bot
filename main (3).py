...
watermark_text = "🅢🅐🅚🅢🅗🅐🅜™"
credit =("🅢🅐🅚🅢🅗🅐🅜™") 
OWNER = int(os.environ.get("OWNER",8399433850))
try: 
    ADMINS=[8399433850] 
    for x in (os.environ.get("ADMINS", "8399433850").split()):  
        ADMINS.append(int(x)) 
except ValueError: 
...
# Define other variables that depend on OWNER
OWNER_ID = 8399433850 # Redundant if OWNER is already defined, but good practice
AUTH_USERS = [OWNER_ID]
AUTH_CHANNEL = -1003110087043

def is_authorized(user_id: int) -> bool:
    return user_id == OWNER_ID or user_id in AUTH_USERS or user_id == AUTH_CHANNEL
...
# --- ClassX Transcoded / Recordings / DB / WSB Handling ---
elif ("https://static-trans-v1.classx.co.in" in url or
      "https://static-trans-v2.classx.co.in" in url):
    base_with_params, signature = url.split("*")
    base_clean = base_with_params.split(".mkv")[0] + ".mkv"
    if "static-trans-v1.classx.co.in" in url:
        base_clean = base_clean.replace("https://static-trans-v1.classx.co.in",
                                        "https://appx-transcoded-videos-mcdn.akamai.net.in")
    elif "static-trans-v2.classx.co.in" in url:
        base_clean = base_clean.replace("https://static-trans-v2.classx.co.in",
    ...
    base_clean = base_clean.replace("https://static-rec.classx.co.in",
                                    "https://appx-recordings-mcdn.akamai.net.in")
    url = f"{base_clean}*{signature}"

elif "https://static-wsb.classx.co.in/" in url:
    clean_url = url.split("?")[0]
    clean_url = clean_url.replace("https://static-wsb.classx.co.in",
